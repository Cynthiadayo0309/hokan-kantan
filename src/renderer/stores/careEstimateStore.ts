...
import type { CareCalculationResult, CareEstimate, CareEstimateInput, PricingVersion, SaveCareDayPayload, SaveCareDaysPayload } from "../../shared/types";
...
export const useCareEstimateStore=defineStore("careEstimate",{state:():State=>({estimate:null,calculation:null,pricingVersion:null,loading:false,message:"",error:""}),actions:{
  ...
  async saveDays(payload:SaveCareDaysPayload){this.error="";this.estimate=await window.hokanApi.saveCareDays(payload);this.calculation=null},

<template>
  <div>
    ...
    <section>
      ...
      <div class="monthly-grid-wrap" style="max-height: 430px">
        <div class="monthly-grid" :style="{ '--day-count': days.length }">
          <div class="grid-cell header left corner">サービス内容</div>
          <div
            v-for="day in days"
            :key="toDateKey(day)"
            :class="['grid-cell', 'header', isWeekend(day) ? 'weekend' : '', isHoliday(day) ? 'holiday' : '', isToday(day) ? 'today' : '']"
          >
            <div>{{ day.getDate() }}</div>
            <div>{{ weekdayLabel(day) }}</div>
          </div>

          <div class="grid-cell left">訪問予定</div>
          <button
            v-for="day in days"
            :key="`${toDateKey(day)}-visit`"
            type="button"
            :class="['grid-cell', 'day-cell', isWeekend(day) ? 'weekend' : '', isHoliday(day) ? 'holiday' : '', isToday(day) ? 'today' : '']"
            @click="openDay(day)"
          >
            <span v-if="summaryFor(toDateKey(day))" class="visit-summary">{{ summaryFor(toDateKey(day)) }}</span>
...
import { daysInMonth, formatMonth, isHoliday, isToday, isWeekend, toDateKey, weekdayLabel } from "../utils/date";
import {
  copaymentOptions,
